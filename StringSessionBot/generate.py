...
async def generate_session(bot, msg, telethon=False):
    ...
    try:
        ...
        return
    try:
        phone_code_msg = await bot.ask(user_id, " 🔍من فضلك افحص حسابك بالتليجرام وتفقد الكود من حساب اشعارات التليجرام. اذا استملته ارسل الكود بعد قراءه هذه الرساله. 
اذا كان الكود `12345`, **🔮من فضلك ارسله لي كالاتي** `1 2 3 4 5`. اذا احتجت مساعده @XxlllllllllllllllllllllllllllxX", filters=filters.text, timeout=600)
        if await cancelled(api_id_msg):
            return
    except TimeoutError:
